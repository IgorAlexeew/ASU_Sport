...
const app = Vue.createApp({
    data() {
        return {
            ...
            update_url: "/api/user/update-clients"
        }
    },
    mounted() {

        document.title = "Клиенты - АГУ СПОРТ"
        axios
...
app.component("clients-table-page", {
    components: {
    ...
    template: `
        <div class="container">
            <data-table title="Клиенты" :select_data="this.$root.select_data" :values="this.$root.values" :headers="this.$root.headers" :update_url="this.$root.update_url"></data-table>
        </div>
    `
})
