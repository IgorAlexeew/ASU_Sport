...
<!--                  <span v-if="this.$root.user.role === 'admin'" class="admin">админстратор</span>-->
                ...
                <p class="birth-date" :class="{editing:this.$root.isEditing}">Дата рождения:&nbsp;<span v-if="!this.$root.isEditing">{{ (new Date(this.$root.user?.dateOfBirth)).toLocaleDateString("ru", {day:"2-digit",month:"2-digit",year:"numeric"}) }}</span>
                  <input v-else type="date" v-model="this.$root.user.dateOfBirth">
<!--                  <span v-if="this.$root.isEditing" class="datepicker-toggle">
    ...
    <!--            <p class="e-mail">dostoevskiy@poet.ru</p>-->
                <p class="phone-number" :class="{editing:this.$root.isEditing}"><div>Номер телефона:&nbsp;</div><input v-model="this.$root.user.phoneNumber" :disabled="!this.$root.isEditing"></p>
            </div>
<!--            <div class="edit">
